...
def loop(j, arr, part_sol_iter, part_sol, qubo_dict, i, parallel, selection, partial_opt, steps, step_layers, step_multiplier, hadof_optimiser, problem_device, qml_device, shots1, shots2, num_qubit, problem_size, num_models, finalsamps, backend, session):
    # Docplex model, we need to convert our problem in this format to use the unbalanced penalization approach
    if parallel == "parallel":
        local_sol = part_sol_iter.copy()
    else:
    ...
    if hadof_optimiser == "QAOAt":
        ...
        # params = pnp.array([[0.5]*step_layers, [0.5]*step_layers], requires_grad=True)
        ...
        qaoa_circuit1, pm, estimator, sampler = QAOAt_qiskit.create_qaoa_circuit(problem_device, num_qubit, backend, session)
    ...
    if parallel != "parallel":
        ...
        if problem_size <= 1000:
            print(f"Step {i+1}/{steps}, Model: {j+1}, Best solution: {part_sol}")
        else:
    ...
    if i == steps-1:
        if hadof_optimiser == "QAOAt":
            ...
            sampleset = sampleset.to_pandas_dataframe().sort_values("energy").reset_index(drop=True)
            samps = sampleset.replace(1, 0).replace(-1, 1)
            for num, row in samps.iterrows():
                if len(samps) == shots2:
            ...
            samps = np.array([list(map(int, s)) for s in bits], dtype=int)
            finalsamps.append(samps)


        return [partial_solution, samps, samples_unbalanced, [res[i].mean() for i in range(num_qubit)]]
    else: 
        return finalsamps, partial_solution